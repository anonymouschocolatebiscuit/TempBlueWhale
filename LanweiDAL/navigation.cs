...
namespace Lanwei.Weixin.DAL
{
    ...
    public partial class navigation
    {
        ...
        public int Add(int parent_id, string nav_name, string title, string link_url, int sort_id, int channel_id, string action_type)
        {
            ...
            //开始赋值
            Lanwei.Weixin.Model.navigation model = new Lanwei.Weixin.Model.navigation();
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Lanwei.Weixin.Model.navigation model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            Lanwei.Weixin.Model.navigation model2 = GetModel(conn, trans, model.parent_id); //带事务
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Lanwei.Weixin.Model.navigation model)
        {
            Lanwei.Weixin.Model.navigation oldModel = GetModel(model.id); //旧的数据
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (IsContainNode(model.id, model.parent_id))
                        {
                            ...
                            {
                                Lanwei.Weixin.Model.navigation oldParentModel = GetModel(conn, trans, oldModel.parent_id); //带事务
        ...
        /// 得到一个对象实体
        /// </summary>
        public Lanwei.Weixin.Model.navigation GetModel(string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Lanwei.Weixin.Model.navigation GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,nav_type,name,title,sub_title,link_url,sort_id,is_lock,remark,parent_id,class_list,class_layer,channel_id,action_type,is_sys");
            strSql.Append(" from " + databaseprefix + "navigation ");
        ...
        /// 得到一个对象实体(重载，带事务)
        /// </summary>
        public Lanwei.Weixin.Model.navigation GetModel(SqlConnection conn, SqlTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,nav_type,name,title,sub_title,link_url,sort_id,is_lock,remark,parent_id,class_list,class_layer,channel_id,action_type,is_sys");
            strSql.Append(" from " + databaseprefix + "navigation ");
        ...
        {
            //查找父节点信息
            Lanwei.Weixin.Model.navigation model = GetModel(conn, trans, parent_id);
            if (model != null)
            {
                //查找子节点
