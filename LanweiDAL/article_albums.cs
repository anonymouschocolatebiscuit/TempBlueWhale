...
namespace Lanwei.Weixin.DAL
{
	...
	public partial class article_albums
	{
        ...
        /// 获得数据列表
        /// </summary>
        public List<Lanwei.Weixin.Model.article_albums> GetList(int article_id)
        {
            List<Lanwei.Weixin.Model.article_albums> modelList = new List<Lanwei.Weixin.Model.article_albums>();
            ...
            if (rowsCount > 0)
            {
                Lanwei.Weixin.Model.article_albums model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Lanwei.Weixin.Model.article_albums();
        ...
        /// </summary>
        public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Lanwei.Weixin.Model.article_albums> models, int article_id)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
                foreach (Lanwei.Weixin.Model.article_albums modelt in models)
                {
                    if (modelt.id > 0)
                    {
        ...
        /// 删除相册图片
        /// </summary>
        public void DeleteFile(List<Lanwei.Weixin.Model.article_albums> models)
        {
            if (models != null)
            {
                foreach (Lanwei.Weixin.Model.article_albums modelt in models)
