...
namespace Lanwei.Weixin.DAL
{
    ...
    public partial class article
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Lanwei.Weixin.Model.article model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            if (model.albums != null)
            {
                ...
                foreach (Lanwei.Weixin.Model.article_albums modelt in model.albums)
                {
            ...
            if (model.attach != null)
            {
                ...
                foreach (Lanwei.Weixin.Model.article_attach modelt in model.attach)
                {
            ...
            if (model.group_price != null)
            {
                ...
                foreach (Lanwei.Weixin.Model.user_group_price models in model.group_price)
                {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Lanwei.Weixin.Model.article model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            StringBuilder strSql5;
                            foreach (Lanwei.Weixin.Model.user_group_price modelt in model.group_price)
                            {
                                strSql5 = new StringBuilder();
                                if (modelt.id > 0)
                                {
        ...
        {
            //取得相册MODEL
            List<Lanwei.Weixin.Model.article_albums> albumsList = new Lanwei.Weixin.DAL.article_albums(databaseprefix).GetList(id);
            //取得附件MODEL
            List<Lanwei.Weixin.Model.article_attach> attachList = new Lanwei.Weixin.DAL.article_attach(databaseprefix).GetList(id);
            ...
            if (rowsAffected > 0)
            {
                new Lanwei.Weixin.DAL.article_albums(databaseprefix).DeleteFile(albumsList); //删除图片
                new Lanwei.Weixin.DAL.article_attach(databaseprefix).DeleteFile(attachList); //删除附件
                return true;
            }
            else
            {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Lanwei.Weixin.Model.article GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            Lanwei.Weixin.Model.article model = new Lanwei.Weixin.Model.article();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Lanwei.Weixin.Model.article GetModel(string call_index)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 获得会员组价格
        /// </summary>
        private List<Lanwei.Weixin.Model.user_group_price> GetGroupPrice(int article_id)
        {
            List<Lanwei.Weixin.Model.user_group_price> ls = new List<Lanwei.Weixin.Model.user_group_price>();
            ...
            if (rowsCount > 0)
            {
                Lanwei.Weixin.Model.user_group_price model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Lanwei.Weixin.Model.user_group_price();
