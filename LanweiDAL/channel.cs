...
namespace Lanwei.Weixin.DAL
{
    ...
    public partial class channel
    {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.channel_fields != null)
                        {
                            ...
                            foreach (Lanwei.Weixin.Model.channel_field modelt in model.channel_fields)
                            {
                        ...
                        if (model.channel_fields != null)
                        {
                            foreach (Lanwei.Weixin.Model.channel_field modelt in model.channel_fields)
                            {
                                Lanwei.Weixin.Model.article_attribute_field fieldModel = new Lanwei.Weixin.DAL.article_attribute_field(databaseprefix).GetModel(modelt.field_id);
                                if (fieldModel != null)
                                {
        ...
        {
            Lanwei.Weixin.Model.channel oldModel = GetModel(model.id); //旧的数据
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.channel_fields != null)
                        {
                            ...
                            {
                                strSql2 = new StringBuilder();
                                Lanwei.Weixin.Model.channel_field fieldModel = null;
        ...
        {
            //取得将要删除的记录
            Lanwei.Weixin.Model.channel model = GetModel(id);
            if (model == null)
            {
                return false;
            }
        ...
        public Lanwei.Weixin.Model.channel GetModel(int id)
        {
            ...
            Lanwei.Weixin.Model.channel model = new Lanwei.Weixin.Model.channel();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds2.Tables[0].Rows.Count > 0)
                {
                    ...
                    List<Lanwei.Weixin.Model.channel_field> models = new List<Lanwei.Weixin.Model.channel_field>();
                    Lanwei.Weixin.Model.channel_field modelt;
                    for (int n = 0; n < i; n++)
                    {
                        modelt = new Lanwei.Weixin.Model.channel_field();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Lanwei.Weixin.Model.channel GetModel(SqlConnection conn, SqlTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,category_id,name,title,is_albums,is_attach,is_group_price,page_size,sort_id from " + databaseprefix + "channel ");
            strSql.Append(" where id=@id ");
        ...
        /// 删除已移除的频道扩展字段
        /// </summary>
        public void FieldDelete(SqlConnection conn, SqlTransaction trans, List<Lanwei.Weixin.Model.channel_field> models, int channel_id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            {
                Lanwei.Weixin.Model.channel_field model = models.Find(p => p.field_id == int.Parse(dr["field_id"].ToString())); //查找对应的字段ID
                if (model == null)
                {
        ...
        /// 删除及重建该频道视图
        /// </summary>
        public void RehabChannelViews(SqlConnection conn, SqlTransaction trans, Lanwei.Weixin.Model.channel model, string old_name)
        {
            //删除旧的视图
            StringBuilder strSql1 = new StringBuilder();
