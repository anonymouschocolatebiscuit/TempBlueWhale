...
namespace Lanwei.Weixin.DAL
{
    public class article_images_size
    {
        ...
        /// </summary>
         public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Lanwei.Weixin.Model.article_images_size> models, int category_id)
         {
             StringBuilder idList = new StringBuilder();
             if (models != null)
             {
                 foreach (Lanwei.Weixin.Model.article_images_size modelt in models)
                 {
                     if (modelt.id > 0)
                     {
         ...
         /// 获得数据列表
         /// </summary>
         public List<Lanwei.Weixin.Model.article_images_size> GetCategoryImageSizeById(int category_id)
         {
             List<Lanwei.Weixin.Model.article_images_size> modelList = new List<Lanwei.Weixin.Model.article_images_size>();
         ...
         /// 得到一个对象实体
         /// </summary>
         public Lanwei.Weixin.Model.article_images_size DataRowToModel(DataRow row)
         {
             Lanwei.Weixin.Model.article_images_size model = new Lanwei.Weixin.Model.article_images_size();
             if (row != null)
             {
