    ...
    public class url_rewrite
    {
        ...
        public bool Add(Lanwei.Weixin.Model.url_rewrite model)
        {
            try
            {
                ...
                //创建子节点
                foreach (Lanwei.Weixin.Model.url_rewrite_item modelt in model.url_rewrite_items)
                {
        ...
        {
            Lanwei.Weixin.Model.url_rewrite model = new Lanwei.Weixin.Model.url_rewrite();
            string filePath = Utils.GetXmlMapPath(MXKeys.FILE_URL_XML_CONFING);
            XmlDocument doc = new XmlDocument();
            doc.Load(filePath);
            XmlNode xn = doc.SelectSingleNode("urls");
            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.Attributes["name"].Value.ToLower() == attrValue.ToLower())
                    {
                        ...
                        //再次遍历子节点
                        List<Lanwei.Weixin.Model.url_rewrite_item> lsItems = new List<Lanwei.Weixin.Model.url_rewrite_item>();
                        foreach (XmlElement xe1 in xe.ChildNodes)
                        {
                            if (xe1.NodeType != XmlNodeType.Comment && xe1.Name.ToLower() == "item")
                            {
                                Lanwei.Weixin.Model.url_rewrite_item item = new Lanwei.Weixin.Model.url_rewrite_item();
        ...
        public Hashtable GetList()
        {
            ...
            List<Lanwei.Weixin.Model.url_rewrite> ls = GetList("");
            foreach (Lanwei.Weixin.Model.url_rewrite model in ls)
            {
                if (!ht.Contains(model.name))
                {
                    foreach (Lanwei.Weixin.Model.url_rewrite_item item in model.url_rewrite_items)
                    {
        ...
        {
            List<Lanwei.Weixin.Model.url_rewrite> ls = new List<Lanwei.Weixin.Model.url_rewrite>();
            string filePath = Utils.GetXmlMapPath(MXKeys.FILE_URL_XML_CONFING);
            XmlDocument doc = new XmlDocument();
            doc.Load(filePath);
            XmlNode xn = doc.SelectSingleNode("urls");
