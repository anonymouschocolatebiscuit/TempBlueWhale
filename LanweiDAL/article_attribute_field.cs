...
namespace Lanwei.Weixin.DAL
{
    ...
    public partial class article_attribute_field
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Lanwei.Weixin.Model.article_attribute_field model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Lanwei.Weixin.Model.article_attribute_field model)
        {
        ...
        {
            //取得Model信息
            Lanwei.Weixin.Model.article_attribute_field model = GetModel(id);
            //开始删除
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            {
                                Lanwei.Weixin.Model.channel modelt = new Lanwei.Weixin.DAL.channel(databaseprefix).GetModel(conn, trans, int.Parse(dr["channel_id"].ToString()));
                                if (modelt != null)
                                {
                                    new Lanwei.Weixin.DAL.channel(databaseprefix).RehabChannelViews(conn, trans, modelt, modelt.name);
                                }
                            }
                        }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Lanwei.Weixin.Model.article_attribute_field GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            Lanwei.Weixin.Model.article_attribute_field model = new Lanwei.Weixin.Model.article_attribute_field();
