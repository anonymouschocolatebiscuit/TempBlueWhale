...
namespace Lanwei.Weixin.DAL
{
    public class UserDAL
    {
        ...
        {
            bool flag = false;
            SqlParameter[] parameters = new SqlParameter[] { new SqlParameter("@phone", SqlDbType.VarChar) };

            string sql = "select * from viewUsers where phone=@phone ";
        ...
        public Users getByUserName(string UserName)
        {
            ...
            if (ds.Tables[0].Rows.Count ==1)
            {
                ...
                user.CorpIdQY = ds.Tables[0].Rows[0]["CorpIdQY"].ToString();
                user.CorpSecretQY = ds.Tables[0].Rows[0]["CorpSecretQY"].ToString();

                user.ShopId = ConvertTo.ConvertInt(ds.Tables[0].Rows[0]["ShopId"].ToString());
                user.ShopName = ds.Tables[0].Rows[0]["shopName"].ToString();
                user.LoginName = ds.Tables[0].Rows[0]["LoginName"].ToString();
                ...
                user.Pwds = ds.Tables[0].Rows[0]["pwds"].ToString();
                user.Names =ds.Tables[0].Rows[0]["names"].ToString();

                user.WeixinId = ds.Tables[0].Rows[0]["WeixinId"].ToString();

                user.AppId = ds.Tables[0].Rows[0]["AppId"].ToString();
                user.AppSecret = ds.Tables[0].Rows[0]["AppSecret"].ToString();

                user.Phone = ds.Tables[0].Rows[0]["phone"].ToString();
                user.MakeDate =DateTime.Parse(ds.Tables[0].Rows[0]["makeDate"].ToString());
        ...
        public int AddQY()
        {
            string sql = @"  if not exists(select * from users where phone=@LoginName )
                      insert into users(corpId,names,deptId,roleId,tel,phone,email,qq,address,brithDay,comeDate,pwd,pwds,makeDate,flag) 
                      values(@shopId,@names,@deptId,@roleId,@tel,@phone,@email,@qq,@address,@brithDay,@comeDate,@pwd,@pwds,@makeDate,@flag)
