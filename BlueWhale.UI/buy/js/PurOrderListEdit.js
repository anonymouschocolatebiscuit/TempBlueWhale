            ...
            columns: [
                {
                    display: '', isSort: false, width: 60, align: 'center', frozen: true, render: function (rowdata, rowindex, value) {
                        ...
                        if (!rowdata._editing) {
                            ...
                            h += "<a href='javascript:f_selectContact()' title='Choose goods' style='float:left;'><div class='ui-icon ui-icon-search'></div></a> ";
                        }
...
function f_onAfterEdit(e) {
    ...
    if (e.column.name == "sumPriceDis")
    {
        ...
        if (sumPriceDis >= num * price) {
            alert("Please fill in coorect result!");
            return;
        }
...
function deleteRow() {
    ...
    if (manager.rows.length == 1) {
        $.ligerDialog.warn('At least left one row!')
    }
    else {
...
function save() {
    ...
    if (bizId == 0) {
        $.ligerDialog.warn('Please select purchaser!');
        return;
    }
    ...
    if (bizDate == "") {
        $.ligerDialog.warn("Fill order date!");
        return;
    }
    ...
    if (sendDate == "") {
        $.ligerDialog.warn("Fill send date!");
        return;
    }
    ...
    if (data.length == 0) {
        $.ligerDialog.warn('Please choose goods!');
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i].num <= 0 || data[i].num == "" || data[i].num == "0" || data[i].num == "0.00") {
            $.ligerDialog.warn("Please insert number" + (i + 1) + "row number of goods!");
    ...
    $.ajax({
        ...
        success: function (jsonResult) {
            if (jsonResult == "Action Complete!") {
                $.ligerDialog.waitting('Action Complete!'); setTimeout(function () { $.ligerDialog.closeWaitting(); location.reload(); }, 2000);
            }
            else {
