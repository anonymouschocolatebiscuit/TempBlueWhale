        ...
        manager = $("#maingrid").ligerGrid({
            columns: [
...

                {
                    ...
                    editor: {
                        type: 'select',
                        url: "../baseSet/LogisticsList.aspx?Action=GetDDLList&r=" + Math.random(),
...
function updateTotal() {
    var data = manager.getData();
    var sumPriceItemDis = 0; var sumPriceItemNow = 0; var sumPriceItemTax = 0; var sumPriceItemAll = 0;
    for (var i = data.length - 1; i >= 0; i--) {
...
function f_onAfterEdit(e) {
    ...
    if (e.column.name == "sumPriceDis") {
        ...
        if (sumPriceDis >= num * price) {
            alert("Please Enter Valid Discound Price!");
            return;
        }
...
function deleteRow() {
    if (manager.rows.length == 1) {
        $.ligerDialog.warn('Atlease 1 row!')
...
function save() {
    ...
    if (venderId == "" || Number(venderId) == 0) {
        $.ligerDialog.warn("Please Select Client!");
        return;
    }
    ...
    if (bizDate == "") {
        $.ligerDialog.warn("Please Select Outbound Date!");
        return;
    ...
    for (var i = 0; i < data.length; i++) {
        if (data[i].num <= 0 || data[i].num == "" || data[i].num == "0" || data[i].num == "0.00") {
            ...
            $.ligerDialog.warn("Please Enter Quantity Row" + (i + 1));
        ...
        if (data[i].ckId == 0 || data[i].ckId == "" || data[i].ckId == "0" || data[i].ckName == "") {
            ...
            $.ligerDialog.warn("Please Enter Storage Row" + (i + 1));
...
function calcDisPrice(dis) {
    ...
    var sumPriceAll = $("#sumPriceItemAll").html();
