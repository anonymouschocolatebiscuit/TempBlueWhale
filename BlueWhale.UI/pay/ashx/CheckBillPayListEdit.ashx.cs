...
namespace BlueWhale.UI.pay.ashx
{
    ...
    public class CheckBillPayListEdit : IHttpHandler, IRequiresSessionState  // Must reference IRequiresSessionState to use Session
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                context.Response.Write("Session timed out, please log in again!");
                return;
            }
            BasePage basePage = new BasePage();
            if (!basePage.CheckPower("CheckBillPayListEdit"))
            {
            ...
            #region Assign values to the main table - Prepayment to accounts payable - typeId=2

            dal.Id = obj.Id;
            dal.ShopId = users.ShopId;
            dal.Number = "";

            dal.BizType = 2; // Prepayment to accounts payable - typeId=2
            ...
            #region Assign values to sub-tables

            if (pId > 0)
            {
                int check = 0;

                #region Insert entries - Prepayment
                ...
                #region Insert entries - Accounts payable
                ...
                if (check > 0)
                {
                    ...
                    logs.Events = "Modified payment verification order: " + obj.Id.ToString();
