...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        $(function ()
        {
            manager = $("#maingrid").ligerGrid({
                columns: [
                 ...
                 {
                     display: 'Picture', name: 'picUrl', id: 'picUrl', width: 50, align: 'center'
                 ...
                 { display: 'Product Quantity', name: 'num', width: 100, type: 'int', align: 'center' },
                 { display: 'Display Order', name: 'seq', width: 60, align: 'center' },
                 {
                     display: 'Mini Program Display', name: 'isShowXCX', id: 'isShowXCX', width: 80, align: 'center'
                     , render: function (rowdata, rowindex, value) {
                         var h = "";
                         if (!rowdata._editing) {
                  ...
                  {
                      display: 'Official Account Display', name: 'isShowGZH', id: 'isShowGZH', width: 80, align: 'center'
                     , render: function (rowdata, rowindex, value) {
                         var h = "";
                         if (!rowdata._editing) {
                 ...
                 { display: 'Action', isSort: false, width: 120,align:'center', render: function (rowdata, rowindex, value)
                 {
                    var h = "";
                    if (!rowdata._editing)
                    {
                ...
                { text: "Modify Category", click:editRow,img: '../lib/ligerUI/skins/icons/modify.gif'},
                { line: true },


                { text: 'Modify Picture', click: editPic, img: '../lib/ligerUI/skins/icons/photograph.gif' },
                { line: true },
        ...
        function editRow()
        {
            ...
            if (!row) {  $.ligerDialog.warn('Please select the row(s) to modify!'); return; }
            var title="Modify Category-"+row.names;
        ...
        function editPic() {
            var row = manager.getSelectedRow();
            if (!row) { $.ligerDialog.warn('Please select the row(s) to modify!'); return; }
            var title = "Modify Category Image-" + row.names;
