...
namespace LanweiBLL
{
	...
	public partial class channel_category
	{
        ...
        private readonly Lanwei.Weixin.DAL.channel_category dal;

		public channel_category()
		{
            dal = new Lanwei.Weixin.DAL.channel_category(siteConfig.sysdatabaseprefix);
        }
		#region 基本方法========================================
        ...
        public bool Exists(string build_path)
        {
            ...
            //与频道名称是否同名
            if (new Lanwei.Weixin.DAL.channel(siteConfig.sysdatabaseprefix).Exists(build_path))
            {
                return true;
            }
            //与频道分类生成目录是否同名
		...
		/// 增加一条数据
		/// </summary>
        public int Add(Lanwei.Weixin.Model.channel_category model)
		{
            int newCategoryId = dal.Add(model);
            if (newCategoryId > 0)
            {
                //添加导航菜单
                int newNavId = new LanweiBLL.navigation().Add("sys_contents", "channel_" + model.build_path, model.title, "", model.sort_id, 0, "Show");
                if (newNavId < 1)
                {
		...
		/// 更新一条数据
		/// </summary>
        public bool Update(Lanwei.Weixin.Model.channel_category model)
		{
            Lanwei.Weixin.Model.channel_category oldModel = dal.GetModel(model.id);
            if (dal.Update(model))
            {
                ...
                //修改对应的导航
                new LanweiBLL.navigation().Update("channel_" + oldModel.build_path, "channel_" + model.build_path, model.title);
		...
		/// 得到一个对象实体
		/// </summary>
		public Lanwei.Weixin.Model.channel_category GetModel(int id)
		{
        ...
        /// 得到一个对象实体
        /// </summary>
        public Lanwei.Weixin.Model.channel_category GetModel(string build_path)
        {
        ...
        public Dictionary<string, string> GetCategoryDirs()
        {
            ...
            List<Lanwei.Weixin.Model.channel_category> modelList = DataTableToList(GetList(0, "", "sort_id asc,id desc").Tables[0]);
            foreach (Lanwei.Weixin.Model.channel_category model in modelList)
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Lanwei.Weixin.Model.channel_category> DataTableToList(DataTable dt)
        {
            List<Lanwei.Weixin.Model.channel_category> modelList = new List<Lanwei.Weixin.Model.channel_category>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Lanwei.Weixin.Model.channel_category model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Lanwei.Weixin.Model.channel_category();
