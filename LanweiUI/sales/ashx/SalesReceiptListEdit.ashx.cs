...
namespace Lanwei.Weixin.UI.sales.ashx
{
    ...
    public class SalesReceiptListEdit : IHttpHandler, IRequiresSessionState  //用Session必须引用IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (basePage.CheckStoreNum())//是否检查负库存
            {
                ...
                for (int i = 0; i < itemList.Rows.Count; i++)
                {
                    ...
                    if (Num > storeNum)//如果大于库存数量
                    {
                        context.Response.Write("第 " + (i + 1).ToString() + " 行商品剩余库存不足，当前库存为：" + storeNum.ToString("0.00"));
                        return;
                    }
