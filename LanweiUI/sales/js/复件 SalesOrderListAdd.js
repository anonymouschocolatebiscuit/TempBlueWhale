    ...
    if (code == 13) {    
        //回车执行查询
         ...
         txtSumPriceSend.set("Width", 100);
            ...
            var  txtSumPriceSendReady=  $.ligerui.get("txtSumPriceSendReady");
         txtSumPriceSendReady.set("Width", 100);
         
         
            var  txtSumPriceSendNeed=  $.ligerui.get("txtSumPriceSendNeed");
         txtSumPriceSendNeed.set("Width", 100);
                ...
                // { display: '主键', name: 'id', width: 50, type: 'int',hide:true},
                 { display: '', isSort: false, width: 60,align:'center',frozen:true, render: function (rowdata, rowindex, value)
                 {
                    ...
                    if (!rowdata._editing)
                    {
                        ...
                        h += "<a href='javascript:f_selectContact()' title='查库存' style='float:left;'><div class='ui-icon ui-icon-search'></div></a> ";
...
//                        h += "<a href='javascript:cancelEdit(" + rowindex + ")'>取消</a> "; 
               ...
               // { display: '商品编码', name: 'code', width: 150, align: 'left',type:'text',hide:true},//editor: { type: 'text' } 
               
                { display: '商品名称', name: 'names', width: 200, align: 'left',
...
//                        }, 
                       ...
                       // onSelected:f_selectContact //f_onGoodsChanged //定义选择了以后的处理方法
                ...
                { display: '规格', name: 'spec',width: 140, align: 'center' },
                
                { display: '单位', name: 'unitName',width: 60, align: 'center' },
                
                { display: '原价', name: 'price', width: 80, type: 'float', align: 'right' },//, editor: { type: 'float', precision: 4 }原价不能修改
                ...
                { display: '折扣%', name: 'dis', width: 70, type: 'float', align: 'center',editor: { type: 'float' } },
                 ...
                 { display: '现价', name: 'priceNow', width: 80, type: 'float', align: 'right', editor: { type: 'float', precision: 4 }
                ...
                { display: '小计', name: 'sumPriceAll', width: 80, type: 'float', align: 'right',
...
　　　　　　            　return "<span id='totalPriceAll'>"+Math.round(Math.round(e.sum*100)/100*100)/100+"</span>";//formatCurrency(suminf.sum)
                ...
                ], width: '99%', pageSizeOptions: [5, 10, 15, 20], height: '300',
                url: 'SalesOrderListAdd.aspx?Action=GetData',
...
        }  
      ...
      function updateTotalSum()
      {
          ...
          //  alert(sumPriceAll);
             ...
             //剩余尾款
            var sumPricePayNeed=Number(sumPriceAll)-Number(sumPricePayReady);
...
function save()
{
     ...
     var headJson={wlId:wlId,sendDate:sendDate,dis:dis,sumPriceDis:sumPriceDis,sumPriceAll:sumPriceAll,sumPriceWY:sumPriceWY,sumPricePayReady:sumPricePayReady,sumPricePayNeed:sumPricePayNeed,sendTypeId:sendTypeId,sendPrice:sendPrice,remarks:remarks,bizId:bizId,bizDate:bizDate};
...
//       $("#txtRemarks").val(JSON.stringify(postData));
         ...
         $.ajax({
            type: "POST",
            url: '../handler/SalesOrderListAdd.ashx',
...
//以下是收款列表
        ...
        $(function ()
        {
            ...
            managersub = $("#maingridsub").ligerGrid({
               ...
               { display: '', isSort: false, width: 40,align:'center',frozen:true, render: function (rowdata, rowindex, value)
                ...
                url: 'SalesOrderListAdd.aspx?Action=GetDataSub',
               rownumbers:true,//显示序号
               frozenRownumbers:true,//行序号是否在固定列中
                dataAction: 'local',//本地排序
                usePager:false,
 ...
 var managersub;
        $(function ()
        {
            ...
            window['gcx'] = 
            managercx = $("#maingridcx").ligerGrid({
                columns: [
                
              
                  { display: '促销活动', name: 'payNumber', width: 150, align: 'left',type:'text',editor: { type: 'text' } }
                
              
                ], width: '200', pageSizeOptions: [5, 10, 15, 20], height: '300',
                url: 'SalesOrderListAdd.aspx?Action=GetDataSub',
               rownumbers:true,//显示序号
               frozenRownumbers:true,//行序号是否在固定列中
                dataAction: 'local',//本地排序
          ...
          var dis= Number(values);
           ...
           if(  $("#txtDis").val()=="")
           {
            ...
            var disPriceAll=Number(sumPriceItem)-Number(sumPriceAll);
             ...
             //剩余尾款
            var sumPricePayNeed=Number(sumPriceAll)-sumPricePayReady;
...
function calcSumSendPrice(values)
{
           ...
           //总运费
            var sumPriceSend=Number(values);
             ...
             //已收运费
           var sumPriceSendReady=Number($("#txtSumPriceSendReady").val());
           
            //待收运费
           var sumPriceSendNeed=sumPriceSend-sumPriceSendReady;
           
           
           //待收运费
           $("#txtSumPriceSendNeed").val(sumPriceSendNeed);
         
}

//已收运费
function calcSumSendPriceReady(values)
{

           //已收运费
            var sumPriceSendReady=Number(values);
