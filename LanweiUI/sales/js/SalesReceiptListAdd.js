       ...
       //新样式引入行
        ...
        {
            $.ligerDialog.open({ title: '选择客户', name:'winselector',width: 800, height:540, url: '../baseSet/ClientListSelect.aspx', buttons: [
         ...
         //扫描开始
                ...
                function keyDownSearch(e) {
                    ...
                    if (code == 13) {
                        ...
                        if (txtKeys != "" && txtKeys != "请用扫描枪或输入后回车……")
                        {
                            ...
                            $.ajax({
                                ...
                                data: "Action=SearchKeys&keysWords=" + txtKeys + "&ranid=" + Math.random(),
                                success: function (resultList) {

                                    if (resultList == "登录超时！")
                                    {
                                        alert(resultList, '提示信息');
                                        $("#txtKeys").val("");
                                        return;
                                    }
                                    ...
                                    var dataList = JSON.parse(resultList);                                                                    
                                    if (dataList.length > 1)
                                    {
                                        alert("此编码有" + dataList.length + "条商品信息，请手动选择！", '提示信息');
                                        $("#txtKeys").val("");
                                        return;

                                    }
                                    addNewRowSearch(dataList);
    ...
    var manager;
        ...
        {
            display: '数量', name: 'num', width: 70, type: 'float', align: 'right', editor: { type: 'float' },
        ...
        { display: '备注', name: 'remarks', width: 150, align: 'left', type: 'text', editor: { type: 'text' } },
                ...
                { display: '源单号', name: 'sourceNumber', width: 180, align: 'left',type:'text' }
        ...
        function addNewRowSearch(e) {
            ...
            manager.addRow({
                ...
                spec: e[0].spec,
                unitName: e[0].unitName,

                num: num,

                price: e[0].priceSalesRetail,
                dis: 0,
                sumPriceDis: 0,
                priceNow: e[0].priceSalesRetail,
                sumPriceNow: e[0].priceSalesRetail * num,
                tax: 0,
                priceTax: e[0].priceSalesRetail,//含税单价
                sumPriceTax: 0,//税额
                sumPriceAll: e[0].priceSalesRetail * num,
        ...
        function f_onGoodsChanged(e) {
            ...
            grid.updateRow(selectedRow, {
                ...
                tax: 0,
                priceTax: selected.priceSalesRetail,//含税单价、默认为不含税单价
                sumPriceTax: 0,//税额
            ...
            if (e.length > 1) //如果有多行的、先删除空白行，然后插入下面
            {
                ...
                for (var i = 1; i < e.length; i++) {
                    grid.addRow({
                        ...
                        tax: 0,
                        priceTax: e[i].priceSalesRetail,//含税单价
                        sumPriceTax: 0,//税额
        ...
        function updateTotal() {
            ...
            var payNow = Number($("#txtPayNow").val());//本次付款
...
//        
      //  return;
         ...
         $.ajax({
            type: "POST",
            url: 'ashx/SalesReceiptListAdd.ashx',
       ...
       function calcPayNo(payNow)
        {
            ...
            var sumPrice = $("#sumPriceItemAll").html();// document.getElementById('<%=txtSumPrice.ClientID %>').value;//折后金额
       ...
       function calcDisPrice(dis) {
           ...
           var sumPriceAll = $("#sumPriceItemAll").html();// document.getElementById('<%=txtSumPrice.ClientID %>').value;//折后金额
