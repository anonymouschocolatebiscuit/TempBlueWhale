        ...
        function selectGet()
        {
           ...
           manager._setUrl("CheckBillGetListAdd.aspx?Action=GetDataListSearchGet&keys=" + keys + "&start=" +start + "&end=" + end+"&wlId="+wlId);
        ...
        function selectBill()
        {
           ...
           managersub._setUrl("CheckBillGetListAdd.aspx?Action=GetDataListSearch&keys=" + keys + "&start=" +start + "&end=" + end+"&wlId="+wlId);
                 ...
                 { display: '', isSort: false, width: 40,align:'center',frozen:true, render: function (rowdata, rowindex, value)
                 {
                    ...
                    if (!rowdata._editing)
                    {
                       // h += "<a href='javascript:addNewRow()' title='新增行' style='float:left;'><div class='ui-icon ui-icon-plus'></div></a> ";
                        h += "<a href='javascript:deleteRow()' title='删除行' style='text-align:center;'><div class='ui-icon ui-icon-trash'></div></a> "; 
                    }
                    else
                    {
...
//                        h += "<a href='javascript:cancelEdit(" + rowindex + ")'>取消</a> "; 
                ...
                { display: '收款编号', name: 'sourceNumber',width: 220, align: 'center',
                ...
                url: 'CheckBillGetListAdd.aspx?Action=GetData',
...
//                        h += "<a href='javascript:cancelEdit(" + rowindex + ")'>取消</a> "; 
                ...
                { display: '应收编号', name: 'sourceNumber',width: 220, align: 'center',
               ...
               url: 'CheckBillGetListAdd.aspx?Action=GetDataSub',
               rownumbers:true,//显示序号
               frozenRownumbers:true,//行序号是否在固定列中
                dataAction: 'local',//本地排序
                usePager:false,
    ...
    //1、先删掉空白行
      ...
      for(var i=0;i<data.length;i++)
     {
         if(Number(data[i].priceCheckNow)<=0 && data[i].sourceNumber!="")
         {
             ...
             $.ligerDialog.warn("请输入付款信息第"+(i+1)+"行的核销金额！");
    ...
    //1、先删掉空白行
      ...
      for(var i=0;i<datasub.length;i++)
     {
         if(Number(datasub[i].priceCheckNow)<=0 && datasub[i].sourceNumber!="")
         {
             ...
             $.ligerDialog.warn("请输入应收信息第"+(i+1)+"行的核销金额！");
...
//       $("#txtRemarks").val(JSON.stringify(postData));
      ...
      // return;
         ...
         $.ajax({
            type: "POST",
            url: 'ashx/CheckBillGetListAdd.ashx',
