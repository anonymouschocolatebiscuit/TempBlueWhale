...
//                        h += "<a href='javascript:cancelEdit(" + rowindex + ")'>取消</a> "; 
                ...
                { display: '付款编号', name: 'sourceNumber',width: 220, align: 'center',
                ...
                url: 'CheckBillPayListEdit.aspx?Action=GetData&id='+getUrlParam("id"),//获取参数
...
//                        h += "<a href='javascript:cancelEdit(" + rowindex + ")'>取消</a> "; 
                ...
                { display: '应付编号', name: 'sourceNumber',width: 220, align: 'center',
               ...
               url: 'CheckBillPayListEdit.aspx?Action=GetDataSub&id='+getUrlParam("id"),//获取参数
               rownumbers:true,//显示序号
               frozenRownumbers:true,//行序号是否在固定列中
                dataAction: 'local',//本地排序
                usePager:false,
    ...
    //1、先删掉空白行
      ...
      for(var i=0;i<datasub.length;i++)
     {
         if(Number(datasub[i].priceCheckNow)<=0 && datasub[i].sourceNumber!="")
         {
             ...
             $.ligerDialog.warn("请输入应付信息第"+(i+1)+"行的核销金额！");
...
//       $("#txtRemarks").val(JSON.stringify(postData));
      ...
      // return;
         ...
         $.ajax({
            type: "POST",
            url: 'ashx/CheckBillPayListEdit.ashx',
