...
function f_selectOrder() {
    $.ligerDialog.open({
        title: '选择生产计划', name: 'winselector', width: 950, height: 600, url: 'produceListSelect.aspx', buttons: [
...
$(function () {
    ...
    //window.open("produceGetListEdit.aspx?Action=GetData&pId=" + param);
    ...
    manager = $("#maingrid").ligerGrid({
        ...
        { display: '单位', name: 'unitName', width: 70, align: 'center' },
          ...
          {
              display: '领料仓库', name: 'ckId', width: 80, isSort: false, textField: 'ckName',
              editor: {
                  type: 'select',
        ...
        {
            display: '申请数量', name: 'numApply', width: 80, type: 'float', align: 'right',
        ...
        {
            display: '实发数量', name: 'num', width: 80, align: 'right', type: 'float',editor: { type: 'float' },
            totalSummary:
             {
                 align: 'right',   //汇总单元格内容对齐方式:left/center/right 
                 type: 'sum',
...
function save() {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].num <= 0 || data[i].num == "") {
            ...
            $.ligerDialog.warn("请输入第" + (i + 1) + "行的领料数量！");
...
function getBomList()
{
    ...
    var pId = $("#hfPId").val();//生产计划ID

    var goodsId = $("#hfGoodsId").val();

    var num = $("#txtNum").val();

   // alert("pId:"+pId+ " goodsId:" + goodsId + " num:" + num);
    ...
    //window.open("produceGetListEdit.aspx?Action=GetDataBom&pId=" + pId + "&goodsId=" + goodsId + "&num=" + num);
