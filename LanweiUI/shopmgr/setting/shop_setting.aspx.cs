...
using Lanwei.Weixin.BLL;


namespace Lanwei.Weixin.Web.shopmgr.setting
{
    public partial class shop_setting : Web.UI.ManagePage
    {
        Lanwei.Weixin.BLL.wx_shop_setting bll = new Lanwei.Weixin.BLL.wx_shop_setting();
        ...
        private void ShowInfo()
        {
            ...
            IList<Lanwei.Weixin.Model.wx_shop_setting> modellist = bll.GetModelList("wId=" + weixin.id);
            if (modellist == null || modellist.Count <= 0)
            {
                return;
            }
            Lanwei.Weixin.Model.wx_shop_setting model = modellist[0];
        ...
        {
            Lanwei.Weixin.Model.wx_shop_setting model = new Lanwei.Weixin.Model.wx_shop_setting();
            try
            {
                Model.wx_userweixin weixin = GetWeiXinCode();
                int wId = weixin.id;
