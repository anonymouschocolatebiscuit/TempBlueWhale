...
namespace Lanwei.Weixin.UI.store.ashx
{
    ...
    public class AssembleListAdd : IHttpHandler, IRequiresSessionState  //用Session必须引用IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (pId > 0)
            {
                ...
                item.PId = pId;
                item.Types = 1;//要组装的商品，所以是入库，数量增加。
                ...
                for (int i = 0; i < itemList.Rows.Count; i++)
                {
                    ...
                    item.PId = pId;
                    item.Types = -1;//被组装的商品，所以是出库，数量减少。
                ...
                if (check > 0)
                {
                    ...
                    logs.Events = "新增商品组装单：" + dal.Number;
